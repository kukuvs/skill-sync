...
export class TerminalSkillSelector implements SkillSelectorPort {
  constructor(private readonly textPrompt: TerminalTextPrompt = new TerminalTextPrompt()) {}
  ...
  async selectMany(items: string[], title: string): Promise<string[]> {
    ...
    if (!input.isTTY || !output.isTTY) {
      return this.selectByNumbers(items, title);
    }

    return this.selectWithKeyboard(items, title);
  }

  private async selectByNumbers(items: string[], title: string): Promise<string[]> {
    output.write(`${title}\n`);
    items.forEach((item, index) => {
      output.write(`${index + 1}. ${item}\n`);
    });

    const answer = await this.textPrompt.ask("Choose numbers separated by comma: ");
  ...
  private async selectWithKeyboard(items: string[], title: string): Promise<string[]> {
